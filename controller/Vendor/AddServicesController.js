...
export const AddServices = async (req, res) => {
  try {
    ...
    const image = req.files
      ? req.files.map(
          (file) =>
            `https://tenalpa-backend.onrender.com/uploads/VendorImage/servicesImage/${file.filename}`
        )
      : [];
    ...
    const lastService = await vendorServices.findOne().sort({ id: -1 }).lean();
    ...
    const newService = new vendorServices({
      id: newId,
      user_id,
      services_category,
      sub_category,
...
export const getAllServices = async (req, res) => {
  try {
    const allServices = await vendorServices.find(); // 👈 sab data le aayega
...
export const GetServicesByvendor = async (req, res) => {
  try {
    ...
    // Fetch all services
    const services = await vendorServices.find({ user_id });
...
export const UpdateServices = async (req, res) => {
  try {
    ...
    const existingService = await vendorServices.findOne({ id: Number(id) });
...
export const DeleteServices = async (req, res) => {
  try {
    ...
    const deletedService = await vendorServices.findOneAndDelete({
      id: Number(id),
    });
