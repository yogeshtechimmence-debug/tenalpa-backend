...
const createMulter = () => {
  const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      ...
      if (type === "USER" && file.fieldname === "image") {
        ...
        uploadPath = path.join(process.cwd(), "uploads/VenderImage/profileImage");
      } else if (type === "VENDOR" && file.fieldname === "previous_job") {
        uploadPath = path.join(process.cwd(), "uploads/VenderImage/previousImage");
      } else {
  ...
  const fileFilter = (req, file, cb) => {
    ...
    allowed.test(ext)
      ? cb(null, true)
      : cb(new Error(" jpeg, jpg, png, webp"));
  };
